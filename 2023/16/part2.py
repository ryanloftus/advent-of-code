...
def bfs(start_i, start_j, start_dir):
    energized = dict()
    energized[(start_i, start_j)] = [start_dir]
    q = Queue()
    q.put((start_i, start_j, start_dir))
...
for i in range(len(lines)):
    max_energized = max(max_energized, bfs(i, 0, "r"), bfs(i, len(lines[0]) - 1, "l"))
for j in range(len(lines[0])):
    max_energized = max(max_energized, bfs(0, j, "d"), bfs(len(lines) - 1, j, "u"))
print(max_energized)
